<x-layout>
    <div class="bg-white mx-auto p-8 rounded-lg shadow-md w-full md:max-w-3xl">
        ...
        <form method="POST" action="{{ route('jobs.update', $job->id) }}" enctype="multipart/form-data">
            ...
            <x-inputs.text-area cols="30" rows="7" id='description' name='description' label='Job Description' placeholder='We are seeking a skilled and motivated Software Developer to join our growing development team...' :value="old('description', $job->description)" />
            ...
            <x-inputs.select id='job_type' name='job_type' label='Job Type' value="{{old('job_type')}}" :options="[
                'Full-Time' => 'Full-Time', 'Part-Time' => 'Part-Time', 'Contract' => 'Contract', 'Temporary' => 'Temporary', 'Internship' => 'Internship', 'Volunteer' => 'Volunteer', 'On-Call' => 'On-Call']" :value="old('job_type', $job->job_type)" />


            <x-inputs.select id='remote' name='remote' label='Remote' :options="[
                0 => 'No', 1 => 'Yes']" :value="old('remote', $job->remote)" />
